...
from ..config import COLL_TRD_PREFIX, TRD_FALLBACK_COLUMNS_WITH_SHIPMENT, TRD_SHEETNAME_PATTERN, TRD_SHIPTIME_KEY, TRD_FALLBACK_COLUMNS, TRD_FILETYPES_ALLOWED, TRD_ID_KEY
...
def extract_date_str(s: pd.Series) -> pd.Series:
    ...
    return s.apply(lambda x: (datetime(1900, 1, 1) + timedelta(days=x)).strftime('%Y-%m'))
...
async def upload_trd(file: UploadFile, year_month: str = None):
    ...
    try:
        ...
        if file.filename.endswith(".xls"):
            ...
            df = pd.read_csv(extract_csv_into_bytes(
                io.BytesIO(file.file.read())), low_memory=False)
        ...
        item["key_map"] = dict((j, i) for (i, j) in columns_map.items())
        ...
        直接暴力删！
        """
        temp_rows = df.__len__()
        df.dropna(inplace=True, axis=0)
        item["nRows"] = df.__len__()
        item["nDeleted"] = temp_rows - item["nRows"]
        print(f"inserting into db with {item['nRows']} rows")
        ...
        # 不指定表名，系统解析每条数据的实际发货时间，精准，但是速度很慢
            ...
            item["nColls"] = list(set(df["coll_name"]))

            assert len(item["nColls"]
                       ) == 1, f"colls too many: {item['nColls']}"
