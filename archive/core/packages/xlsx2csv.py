...
class Xlsx2csv:
    ...
    def __init__(self, xlsxfile, **options):
        ...
        try:
            ...
            print("failed to init 1")
            raise InvalidXlsxFileException(
                "Invalid xlsx file: " + str(xlsxfile))
        except Exception as e:
            print("failed to init 2")
            print(e.args)
            raise e
        ...
        self.workbook = self._parse(
            Workbook, self.content_types.types["workbook"])
        workbook_relationships = list(
            filter(lambda r: "book" in r, self.content_types.types["relationships"]))[0]
    ...
    def getSheetIdByRegexName(self, regex_name):
        for s in self.workbook.sheets:
            if re.match(regex_name, s['name']):
                return s['index']
        return None
            ...
            for s in self.workbook.sheets:
                ...
                if len(include_sheet_pattern) > 0:
                    ...
                    for pattern in include_sheet_pattern:
                        include = pattern and len(
                            pattern) > 0 and re.match(pattern, sheetname)
                        if include:
                            break
                    if not include:
                        continue
                ...
                if isinstance(outfile, str):
                    ...
                    of.write(self.options['sheetdelimiter'] + " " + str(
                        s['index']) + " - " + sheetname + self.options['lineterminator'])
    ...
    def _convert(self, sheet_index, outfile):
        ...
        if isinstance(outfile, str):
            if sys.version_info[0] == 2:
                ...
                sys.stderr.write(
                    "error: version of your python is not supported: " + str(sys.version_info) + "\n")
                sys.exit(1)
            closefile = True
        try:
            ...
            if len(sheets_filtered) == 0:
                eprint("Sheet with index %i not found or can't be handled" %
                       sheet_index)
                return 1
            ...
            if sheet_file is None:
                raise SheetNotFoundException(
                    "Sheet %i not found" % sheet_index)
            sheet = Sheet(self.workbook, self.shared_strings,
                          self.styles, sheet_file)
            try:
                relationships_path = os.path.join(os.path.dirname(sheet_path),
                                                  "_rels",
                                                  os.path.basename(sheet_path) + ".rels")
                sheet.relationships = self._parse(
                ...
                sheet.set_skip_trailing_columns(
                    self.options['skip_trailing_columns'])
...
class Workbook:
    ...
    def parse(self, filehandle):
        ...
        if workbookDoc.firstChild.namespaceURI:
            ...
            fileVersion = workbookDoc.firstChild.getElementsByTagName(
                "fileVersion")
        if len(fileVersion) == 0:
            ...
            try:
                if workbookDoc.firstChild.namespaceURI:
                    self.appName = workbookDoc.firstChild.getElementsByTagNameNS(
                        workbookDoc.firstChild.namespaceURI, "fileVersion")[0]._attrs['appName'].value
                else:
                    self.appName = workbookDoc.firstChild.getElementsByTagName("fileVersion")[
        ...
        try:
            if workbookDoc.firstChild.namespaceURI:
                self.date1904 = workbookDoc.firstChild.getElementsByTagNameNS(
                    workbookDoc.firstChild.namespaceURI, "workbookPr")[0]._attrs['date1904'].value.lower().strip() \
                    != "false"
            else:
                self.date1904 = workbookDoc.firstChild.getElementsByTagName("workbookPr")[0] \
        ...
        if workbookDoc.firstChild.namespaceURI:
            sheets = workbookDoc.firstChild.getElementsByTagNameNS(
                workbookDoc.firstChild.namespaceURI, "sheets")[0]
        else:
...
class ContentTypes:
    ...
    def parse(self, filehandle):
        ...
        if types.namespaceURI:
            overrideNodes = types.getElementsByTagNameNS(
                types.namespaceURI, "Override")
        else:
...
class Relationships:
    ...
    def parse(self, filehandle):
        ...
        if doc.namespaceURI:
            ...
            relationshipNodes = relationships[0].getElementsByTagName(
                "Relationship")
            ...
            for cellXfs in cellXfsElement[0].childNodes:
                ...
                if cellXfs._attrs and 'numFmtId' in cellXfs._attrs:
                    ...
                    if self.chk_exists(numFmtId) == None:
                        numFmtId = int(
                            cellXfs._attrs['applyNumberFormat'].value)
...
class Sheet:
    ...
    def set_merge_cells(self, mergecells):
        ...
        if doc.namespaceURI:
            mergeCells = doc.getElementsByTagNameNS(
                doc.namespaceURI, "mergeCell")
        else:
    ...
    def set_include_hyperlinks(self, hyperlinks):
        ...
        doc = minidom.parseString(worksheet + data + "</worksheet>").firstChild
        if doc.namespaceURI:
            hiperlinkNodes = doc.getElementsByTagNameNS(
                doc.namespaceURI, "hyperlink")
        else:
    ...
    def handleCharData(self, data):
        if self.in_cell_value:
            ...
            if self.colType == "s":  # shared string
                ...
                format_type = "boolean"
                self.data = (int(data) == 1 and "TRUE") or (
                    int(data) == 0 and "FALSE") or data
                ...
                # get format type
                if not format_str:
                    eprint("unknown format %s at %d" %
                           (format_str, xfs_numfmt))
                    return
            ...
            if format_type and not format_type in self.ignore_formats:
                try:
                    if format_type == 'date':  # date/time
                        if self.workbook.date1904:
                            date = datetime.datetime(
                                1904, 1, 1) + datetime.timedelta(float(self.data))
                        else:
                            date = datetime.datetime(
                                1899, 12, 30) + datetime.timedelta(float(self.data))
                        if self.dateformat:
                    ...
                    # if cell is general, be aggressive about stripping any trailing 0s, decimal points, etc.
                        ...
                        self.data = ("%f" % (float(self.data))
                                     ).rstrip('0').rstrip('.')
                    elif format_type == 'float' and format_str[0:3] == '0.0':
                        if self.floatformat:
                            self.data = str(
                                self.floatformat) % float(self.data)
                        else:
    ...
    def handleStartElement(self, name, attrs):
        ...
        if self.in_row and (name == 'c' or (has_namespace and name.endswith(':c'))):
            ...
            if 'spans' in attrs:
                self.spans = [int(i)
                              for i in attrs['spans'].split(" ")[-1].split(":")]
        elif name == 't':
        ...
        if self.in_cell and ((name == 'v' or name == 'is' or name == 't') or (
            ...
            if self.colNum + self.rowNum in self.mergeCells.keys():
                if 'copyFrom' in self.mergeCells[self.colNum + self.rowNum].keys() and \
                    ...
                    d = self.mergeCells[self.mergeCells[self.colNum +
                                                        self.rowNum]['copyFrom']]['value']
...
if __name__ == "__main__":
    ...
    if "ArgumentParser" in globals():
        ...
        parser.add_argument('infile', metavar='xlsxfile',
                            help="xlsx file path")
        parser.add_argument('outfile', metavar='outfile',
                            nargs='?', help="output csv file path")
    ...
    try:
        if os.path.isdir(options.infile):
            ...
            if options.sheetname:
                ...
                if not sheetid:
                    raise XlsxException("Sheet '%s' not found" %
                                        options.sheetname)
