...
from config import MONGO_COLL_DETAIL_NAME, MONGO_COLL_STAT_NAME
from const import ErrorType, ALLOWED_FILE_STANDS, TRD_COLUMN_FALLBACK_MAP, ERP_COLUMN_MAP, \
...
@app.get("/db/query/list")
async def query_db(
        limit: int = Query(10),
        skip: int = Query(0),
):
    items = db[MONGO_COLL_DETAIL_NAME].find({}, limit=limit, skip=skip)
    return {
        "status": "ok",
        "data": list(items)
    }
